...
BUNDLE_PATTERN = re.compile(r"御取引条件|継続取引|新規・継続|取引申請書類|取引申請書(?!.*?)$")

# 単一内容PDFキーワード（優先度順：具体的なものほど上）
SINGLE_CONTENT_RULES = [
    (re.compile(r"^建設業許可|^建築業許可|^許可証(?!明書)|^許可通知|^許可　"), "建設業許可証"),
    (re.compile(r"^会社案内|^会社概要|^会社プロフィール"), "会社案内"),
    (re.compile(r"^工事経歴|^施工実績|^令和\d+年工事経歴"), "工事経歴書"),
    (re.compile(r"^労働者名簿|^従業員名簿"), "労働者名簿"),
    (re.compile(r"^(主要)?取引先|^得意先|^仕入先|^.*取引先一覧"), "取引先一覧表"),
    (re.compile(r"^労働安全衛生誓約|^誓約書"), "労働安全衛生誓約書"),
    (re.compile(r"^資格者名簿|^有資格者|^技能者名簿"), "資格略字一覧"),
    # 決算は最後（「決算公告用」「簡易版」「報告書」各種含む）
    (re.compile(r"^.*?決算|^.*?B/?S|^.*?P/?L|損益計算書|貸借対照表|^第\d+期"), "決算書"),
]
