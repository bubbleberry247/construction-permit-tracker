...
藤田軸 (有効/書類不備/未提出/期限90日以内/期限切れ/対象外) を機械的に算出。

入力: 当方 v2 一覧表 + DB (companies, permits, company_doc_exemptions, MLITPermits 同期)
...
※ MLIT 期限が取得できない場合は添付許可証期限 (v2 一覧表の 有効期限列) を使用
...
def parse_date(v) -> date | None:
    ...
    if not s:
        return None
    # 和暦 (R8.4.6 等) は対応しない (MLIT 同期側で ISO 化されている前提)
...
def classify(row: dict, db_status: str | None, exempts: set[str], today: date) -> dict:
    ...
    required = 8 - len(exempts)

    if received == 0:
        return {"status": "未提出", "reason": "全 8 書類受領なし"}
    if received >= required:
        if exempts:
            return {"status": "有効", "reason": f"{received}/{required} (除外 {len(exempts)})"}
        return {"status": "有効", "reason": f"{received}/8 全揃い"}
    return {"status": "書類不備", "reason": f"{received}/{required} (不足 {required-received})"}
