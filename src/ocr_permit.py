...
- company_name_raw: 会社名（原文のまま）
- permit_number_full: 許可番号（例: 愛知県知事 許可（特一-6）第57805号）
- permit_authority_name: 許可行政庁名（例: 愛知県知事）
- permit_category: "一般" または "特定"
- permit_year: 許可年度（整数）
- contractor_number: 業者番号（例: "57805"）
- trade_categories: 建設業種リスト（例: ["土木工事業", "建築工事業"]）
- issue_date: 許可年月日（YYYY-MM-DD形式または和暦表記そのまま）
- expiry_date: 有効期限（YYYY-MM-DD形式または和暦表記そのまま）
- note: その他特記事項

document_typeが"建設業許可証"以外の場合は、上記フィールドはnullとしてください。
...
def _validate_and_hash(
    ...
    if file_size > 20 * 1024 * 1024:
        result["error_category"] = TIER1_VALIDATION_ERROR
        result["error_reason"] = f"ファイルサイズ超過: {file_size // 1024 // 1024}MB > 20MB"
        return False
...
def _extract_and_check_confidence(
    ...
    """テキスト抽出 + 信頼度チェック。失敗は result に記録して ("", False) を返す。"""
    try:
    ...
    if confidence < low_thresh:
        ...
        result["error_reason"] = f"OCR信頼度が低すぎます: {confidence:.2f} < {low_thresh}"
        return "", False
...
# ---------------------------------------------------------------------------
def _call_llm(
    ocr_text: str, config: dict[str, Any], result: dict[str, Any]
) -> tuple[dict[str, Any], bool]:
    """GPT-4o 呼び出し。失敗は result に記録して ({}, False) を返す。"""
    ...
    try:
        extracted, retry_count = call_with_retry(
            lambda: call_gpt4o_extract(ocr_text, model, api_key),
...
def process_pdf(
    ...
    ocr_text, ok = _extract_and_check_confidence(pdf_path, config, result)
    if not ok:
        return result

    extracted, ok = _call_llm(ocr_text, config, result)
    if not ok:
        return result
